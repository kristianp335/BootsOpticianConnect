...
(function() {
    ...
    function initializeDataTable() {
        ...
        const dataTable = container.querySelector('.sp-data-table-fragment');
        ...
        // Get configuration
        const table = container.querySelector('.sp-data-table');
    ...
    function initializePagination() {
        const container = (typeof fragmentElement !== 'undefined') ? fragmentElement : document;
        const prevBtn = container.querySelector('.sp-pagination-controls .sp-btn:first-child');
        const nextBtn = container.querySelector('.sp-pagination-controls .sp-btn:last-child');
    ...
    function loadSampleData(tableType) {
        const container = (typeof fragmentElement !== 'undefined') ? fragmentElement : document;
        const tbody = container.querySelector('.sp-data-table tbody');
        ...
        // Show loading state
        tbody.innerHTML = '<tr><td colspan="100%" class="sp-table-loading">Loading data...</td></tr>';
    ...
    function renderTable() {
        ...
        if (filteredData.length === 0) {
            tbody.innerHTML = '<tr><td colspan="100%" class="sp-table-empty">No data found</td></tr>';
            return;
        }
        ...
        const rows = pageData.map(function(item) {
            ...
            const actionCell = showActions !== null ? '<td><div class="sp-table-actions"><button class="sp-btn sp-btn-outline">View</button><button class="sp-btn sp-btn-outline">Edit</button></div></td>' : '';
            ...
            return '<tr>' + 
                values.map(function(value) {
                    if (typeof value === 'string' && (value.includes('Active') || value.includes('Completed') || value.includes('Pending') || value.includes('Resolved') || value.includes('Cancelled') || value.includes('Failed'))) {
                        const statusClass = 'sp-status-' + value.toLowerCase().replace(' ', '-');
                        return '<td><span class="sp-status ' + statusClass + '">' + value + '</span></td>';
                    }
                    return '<td>' + value + '</td>';
                }).join('') + 
                actionCell + 
    ...
    function updatePagination() {
        const container = (typeof fragmentElement !== 'undefined') ? fragmentElement : document;
        const startRowSpan = container.querySelector('.sp-start-row');
        const endRowSpan = container.querySelector('.sp-end-row');
        const totalRowsSpan = container.querySelector('.sp-total-rows');
        const prevBtn = container.querySelector('.sp-pagination-controls .sp-btn:first-child');
        const nextBtn = container.querySelector('.sp-pagination-controls .sp-btn:last-child');
